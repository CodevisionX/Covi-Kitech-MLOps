...
export class Deploy implements OnInit, OnDestroy {
  ...
  loadActiveDeployments() {
    this.deploymentService.getActiveDeployments().subscribe({
      next: (list) => this.activeDeployments.set(list),
      error: () => this.notificationService.showError('배포 목록을 가져오지 못했습니다.')
    });
  }
  ...
  viewLogs(dep: IDeployment) {
    this.dialog.open(TerminalLog, {
      ...
      data: {
        ...
        type: 'deployment' // TerminalLog 컴포넌트 내부에서 getDeploymentLogStream 호출하도록 설계
      }
    });
  }
